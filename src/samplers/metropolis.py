...
class MetropolisVMC2:
    ...
    def _rw_metropolis_step(self, positions, logp_current, alpha):
        ...
        proposals = self._rng.normal(loc=positions, scale=self._scale)
        logp_proposal = self._logp(proposals, alpha)
        accept = log_unif < logp_proposal - logp_current
        # Where accept is True, yield proposal, otherwise yield old position
        positions = np.where(accept, proposals, positions)
        logp_current = np.where(accept, logp_proposal, logp_current)
    ...
    def _tune_optimize(self, tune_iter, tune_interval, positions, alpha):
        ...
        for i in range(tune_iter):
            ...
            if steps_before_tune == 0:
                ...
                # early stopping?
                dL2_scale = np.linalg.norm(self._scale - scale_old)
                dL2_alpha = np.linalg.norm(alpha - alpha_old)
