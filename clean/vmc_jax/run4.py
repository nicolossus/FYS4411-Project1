...
class JaxWF:
    ...
    @partial(jit, static_argnums=(0,))
    def HPsi(self, r, alpha):
        rd = r**(self.dim - 1)
        lap = 1 / rd * grad(rd * grad(self.evaluate)(r, alpha))(r, alpha)
        K = -0.5 * lap
        V = 0.5 * self._omega2 * r**2
        return K + V

    @partial(jit, static_argnums=(0,))
    def locE(self, r, alpha):
        r_flat = jnp.sqrt(jnp.sum(r**2, axis=1))
        E_loc = self.HPsi(r_flat, alpha) / self.evaluate(r, alpha)
        return E_loc
...
wf = JaxWF(N, d, omega)
alpha = 0.5
r = safe_initial_state(wf, alpha, 7)
E = wf.locE(r, alpha)
print(E)
'''
